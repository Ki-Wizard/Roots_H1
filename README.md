...
- 스토리형 딜레마 질문 40개 실제 동작
- 4개 사고축의 결정론적 점수 계산
- 대표·보조 철학 유형과 대표 철학자 매칭
- 답변 근거, 반대 관점, 사고 훈련, 공유 문구를 포함한 Mock 또는 OpenRouter 리포트
- 시작 → 설문 → 분석 로딩 → 결과 카드의 반응형 React 화면
- 선택적 실제 AI 호출과 서버 영구 저장 없음
...
backend/     Express API, 점수 계산, Mock/OpenRouter 리포트
...
1. A/B 선택은 질문별 가중치와 함께 코드로 계산합니다.
